...
func TestHighLatencyPopulationStillScoresWell(t *testing.T) {
	c := DefaultCriteria()
	cands := []*Candidate{
		Evaluate(httpResult("1.0.0.1", 710*time.Millisecond, true, true, 5_000_000, 3), cleanRep(), c),
		Evaluate(httpResult("1.0.0.2", 780*time.Millisecond, true, true, 5_000_000, 3), cleanRep(), c),
		Evaluate(httpResult("1.0.0.3", 830*time.Millisecond, true, true, 5_000_000, 3), cleanRep(), c),
	}
	Rank(cands)
	...
	// Before relative scoring these addresses scored in the 60s despite being
	// clean, stable and fast; the best of a healthy population should be high.
	if best.Total < 85 {
...
func TestNearIdenticalLatenciesAreNotAmplified(t *testing.T) {
	c := DefaultCriteria()
	cands := []*Candidate{
		Evaluate(httpResult("2.0.0.1", 700*time.Millisecond, true, true, 2_000_000, 3), cleanRep(), c),
		Evaluate(httpResult("2.0.0.2", 705*time.Millisecond, true, true, 2_000_000, 3), cleanRep(), c),
	}
	Rank(cands)
